...
const DashboardSingleUserPage = ({
  ...
  toast.error("Format alamat email yang Anda masukkan tidak valid");
        return;
      }
      ...
      if (userInput.newPassword.length > 7) {
        ...
        apiClient.put(`/api/users/${id}`, requestOptions)
          ...
          .then((data) => toast.success("Pengguna berhasil diperbarui"))
          .catch((error) => {
            toast.error("Terjadi kesalahan saat memperbarui pengguna");
          });
      } else {
  toast.error("Kata sandi harus lebih dari 7 karakter");
        return;
      }
    } else {
  toast.error("Untuk memperbarui pengguna Anda harus mengisi semua nilai");
      return;
    }
  };
