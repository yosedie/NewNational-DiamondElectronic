...
const getAllProducts = asyncHandler(async (request, response) => {
  ...
  if(mode === "admin"){
    ...
    // Check for categoryId parameter directly
    if (request.query.categoryId) {
      categoryIdFilter = request.query.categoryId;
      console.log('Category ID filter from query:', categoryIdFilter);
    }
    ...
    if (filterObj.category && filterObj.category.equals) {
      categoryFilter = filterObj.category.equals;
    ...
    // Add categoryId filter if it exists (priority over category name filter)
    if (categoryIdFilter) {
      finalWhereClause.categoryId = categoryIdFilter;
      console.log('Filtering by categoryId:', categoryIdFilter);
    } else if (categoryFilter) {
      // Fallback to category name filter
      finalWhereClause.category = {
        name: {
    ...
    products = await prisma.product.findMany({
      ...
      ...(hasFilters && { where: finalWhereClause }),
      orderBy: sortObj,
    });

    console.log(`Found ${products.length} products with filters:`, finalWhereClause);
