...
figma.ui.onmessage = async (msg) => {
  if (msg.type === 'generate-all') {
    ...
    await generateAtoms();
    figma.notify('Atomic components generated! ✅');
  } else if (msg.type === 'generate-molecules') {
    await generateMolecules();
    figma.notify('Molecular components generated! ✅');
  } else if (msg.type === 'generate-organisms') {
    await generateOrganisms();
    figma.notify('Organism components generated! ✅');
  } else if (msg.type === 'generate-pages') {
    await generatePages();
    figma.notify('Page templates generated! ✅');
  } else if (msg.type === 'generate-shop-page') {
    await generateShopPage();
    figma.notify('Shop page generated! ✅');
  } else if (msg.type === 'setup-styles') {
    await setupDesignTokens();
    figma.notify('Design tokens and styles created! ✅');
...
async function generateAllComponents() {
  ...
  await generateAtoms();
  figma.notify('Atoms created! ✅');
  
  await generateMolecules();
  figma.notify('Molecules created! ✅');
  
  await generateOrganisms();
  figma.notify('Organisms created! ✅');
  
  await generatePages();
  figma.notify('Pages created! ✅');
  
  figma.notify('🎉 All components generated successfully!');
}
